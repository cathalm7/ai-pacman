...
# Graphics display code for Pacman game
...
class PacmanGraphics:
    ...
    def update(self, newState):
        ...
        # No additional delay needed - timing is handled in animation
    ...
    def animatePacman(self, pacman, prevPacman, image):
        if self.frameTime < 0:
            ...
            if "q" in keys:
                self.frameTime = 0.1
        # Always use smooth animation for better gameplay
        fx, fy = self.getPosition(prevPacman)
        px, py = self.getPosition(pacman)
        frames = 3.0  # Fewer frames for smoother movement
        for i in range(1, int(frames) + 1):
            ...
            refresh()
            sleep(0.05)  # Short, consistent delay for smooth movement
        refresh()
